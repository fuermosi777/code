...
Array.prototype.hasMoreThanOneZero = function() {
  let last;
  for (let i = 0; i < this.length; i++) {
    if (this[i] === 0 && last === 0) return true;
    last = this[i];
  }
  return false;
};
...
var checkSubarraySum = function(nums, k) {
  ...
  if (nums.hasMoreThanOneZero() || k === 1) {
    return true;
  } 
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    let m = 1;
    while (sum - k * m >= 0) {
      if (map.hasOwnProperty(sum - k * m) && i - map[sum - k * m] >= 2) {
        return true;
      }
      if (k !== 0 && k !== 1) {
        m++;
